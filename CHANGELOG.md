...
- 邮件页联系人展示逻辑改为完整加载联系人分页数据，并建立邮箱前缀唯一匹配索引；邮件头只有 `zhanghtb` 这类账号名或联系人排在第一页之后时，也能优先显示已维护的昵称、显示名称或姓名，最后才回退到邮件地址/原始名称。
- 邮箱账号页表格优化为“账号信息 + 服务器信息 + 状态 + 同步 + 操作”的紧凑结构，账号名称和邮箱地址合并展示，IMAP/SMTP 主机合并展示，并增加表格横向滚动与固定操作列，避免窄屏下名称被挤成竖排。
- 邮件页恢复与其他页面一致的外层工作区圆角、边框和背景框架，右侧邮件详情区增加统一的面板背景和内部边界，避免看起来像脱离应用框架的纯白区域。
- 系统管理用户列表增加 30 秒短 TTL 内存缓存，避免频繁打开页面或刷新时重复聚合用户邮件、附件、联系人、文件夹和规则统计；用户创建、资料、头像、密码和启停状态变更后会主动清理缓存。
- 系统管理页首次加载时改为显示统计骨架和明确的“正在加载用户数据”状态，刷新按钮进入 loading，并给用户表格增加横向滚动，减少接口偶发慢时的空白感和布局挤压。
- 后端系统管理用户列表接口增加中文耗时日志，生产排查时可直接从容器日志看到接口执行耗时和返回用户数量。

### 测试

- 新增后端回归测试，覆盖系统管理用户统计缓存返回副本且用户状态更新后会失效。
- 前端 `rtk npm run build` 通过，后端 `rtk go test ./...` 通过。

### 部署

- 将前端版本 `20260731091600-983ad03-contact-name` 部署到生产环境，后端继续使用 `20260731085019-983ad03-admin-cache`；按 L1 轻量备份策略保存远端 Compose、配置、旧镜像标签和回滚说明到 `backups/deploy-light/20260731091600-contact-name/`，未全量压缩 `data/`；线上健康检查、`/mail`、前端容器镜像标签和 Nginx 启动日志验证通过。
- 按生产备份保留策略清理更早的 L1 自动备份，`backups/deploy-light/` 目前保留 `20260731090324-layout-polish` 和 `20260731091600-contact-name`。
- 将前端版本 `20260731090324-983ad03-layout-polish` 部署到生产环境，后端继续使用 `20260731085019-983ad03-admin-cache`；按 L1 轻量备份策略保存远端 Compose、配置、旧镜像标签和回滚说明到 `backups/deploy-light/20260731090324-layout-polish/`，未全量压缩 `data/`；线上健康检查、`/accounts`、`/mail`、前端容器镜像标签和 Nginx 启动日志验证通过。
- 按生产备份保留策略清理更早的 L1 自动备份，`backups/deploy-light/` 目前保留 `20260731085019-admin-cache` 和 `20260731090324-layout-polish`。
- 将前后端版本 `20260731085019-983ad03-admin-cache` 部署到生产环境；按 L1 轻量备份策略保存远端 Compose、配置、旧镜像标签和回滚说明到 `backups/deploy-light/20260731085019-admin-cache/`，未全量压缩 `data/`；线上健康检查、前端系统管理路由、管理员接口未登录拦截、前后端容器镜像标签和后端启动日志验证通过。
- 当次部署后按生产备份保留策略清理更早的 L1 自动备份，`backups/deploy-light/` 保留 `20260730141855-cleanup-uid` 和 `20260731085019-admin-cache`。

## 2026-07-30

### 运维

- 将本地目录 `/Volumes/ligson-ssd-m2/Downloads/mailback/政务公司发文/` 的 608 个 EML 通过 NAS 侧受控目录导入工具写入生产环境 `ligson` 用户的 `ligson@aliyun.com` 收件箱（`INBOX`）；新增 597 封、去重 11 封、失败 0 封，并保存 3193 个附件。
- 导入完成后回查新增邮件的 `sent_at` 均已解析，日期范围为 2019-05-31 至 2026-02-23；已清理 NAS 与后端容器内的临时压缩包、解压目录和导入工具副本。
...
- 修复全量同步完成后执行“删除服务器旧邮件”时，可能把本地 EML 导入邮件的 `eml-*` UID 当作 IMAP 服务器 UID 处理，导致任务报错 `IMAP UID 格式错误` 的问题；服务器清理候选现在排除导入 UID，并在删除前兜底校验 UID 必须为纯数字。
...
- 新增后端回归测试，覆盖服务器旧邮件清理候选会跳过本地 EML 导入 UID，只保留纯数字 IMAP UID。
- 前端 `rtk npm run build` 通过，后端 `rtk go test ./...` 通过。
- 生产环境补导回查通过：18 个未完成上传会话已全部同步为 `imported`，误导入 AppleDouble 邮件剩余数量为 0。

### 部署

- 将后端版本 `20260730141855-983ad03-cleanup-uid` 部署到生产环境；按 L1 轻量备份策略保存远端 Compose、配置、旧镜像标签和回滚说明到 `backups/deploy-light/20260730141855-cleanup-uid/`，未全量压缩 `data/`；线上健康检查和后端容器镜像标签验证通过。
...
- 浏览器端 EML 导入新增上传会话和断点续传接口：单个文件按 16MB 分片上传，支持心跳查询、断点恢复、偏移量冲突返回服务端真实进度，适合十几 GB 的大批量导入。
- 前端 EML 导入改为逐个文件上传，不再依赖一个长时间 multipart 请求；重新选择同一批文件后可从服务端已接收位置继续上传。

### 优化

- EML 导入窗口的进度提示改为“文件级 + 分片级”展示，并保留上传速度、预计剩余时间和失败明细，避免长任务中断后丢失提示。
- 上传会话元数据与临时 EML 文件保存在持久化数据目录中，导入成功后自动清理临时原文，重复 finish 会直接返回已完成结果。

### 测试

- 新增后端测试覆盖上传会话创建、分片上传、心跳查询、偏移量冲突、完成导入和重复 finish 幂等。
- 前端 `rtk npm run build` 通过，后端 `rtk go test ./...` 通过。
...
- 邮箱账号管理页新增“删除日志”入口，可查看服务器旧邮件清理的已删除、已跳过、失败和待确认记录。

### 安全

- 收紧服务器旧邮件清理条件：只有本地数据库已有记录、目录为 `INBOX`、超过保留天数、并且 NAS 数据目录中 `raw.eml` 原文文件真实存在的邮件 UID 才会提交给 IMAP 删除；原文缺失的旧邮件会跳过并写入日志。
- IMAP 删除失败时会把本批次候选日志更新为失败并保留错误原因，便于排查远端删除是否真正执行。

### 文档

- 更新 API 和架构文档，补充服务器删除日志表、查询接口和同步清理安全确认规则。
...
- 优化超大量 EML 导入体验：前端取消选择文件总大小限制，改为约 512MB 一批自动拆分上传，导入弹窗展示总进度、当前批次、已上传大小、已处理文件数和新增/重复/失败统计，导入过程中禁止误关弹窗，单批上传失败会自动重试一次。
- EML 分批导入进度补充实时上传速度和预计剩余时间，文件总大小超过 1GB 时改用 GB 单位展示，便于观察十几 GB 导入任务的耗时。
- EML 导入失败信息改为固定显示在导入窗口内：整批失败会显示失败批次、上传进度、速度和错误原因；部分文件失败会展示失败文件明细，避免长时间上传后错过短暂提示。
- 优化邮件页列表顶部工具栏：标题与邮件/会话切换合并为紧凑标题行，写信、收取、刷新和导入入口统一按钮高度与左对齐布局，刷新和导入改为图标按钮，减少窄列表栏下的多行留白和视觉杂乱。
- 放宽批量 EML 导入限制：取消文件个数硬限制，单个文件和单次导入文件总大小提升到 2GB；后端 multipart 解析改为小内存缓存加临时文件承载，并按文件逐个读取、导入和释放，前端导入请求取消 Axios 超时限制，容器 nginx 上传上限提升到 `2304m` 并对 `/api/` 开启流式转发和长读写超时。

### 修复

- 修复发送记录、邮件列表、邮件详情和会话参与人展示地址时可能直接显示 RFC 2047 编码词的问题，历史记录读取时会统一转为可读的 `姓名 <邮箱>` 格式。
- 修复批量导入 EML 时多个文件总大小超过前端容器 nginx `50m` 限制会返回 `413` 的问题，前端容器上传上限提升到 `256m`，并在导入弹窗增加数量、单文件大小和总大小校验及中文错误提示。
...
- 更新 API、需求、架构和后端包结构文档，补充附件预览接口、EML 导入接口、支持范围、去重策略和规则触发来源。
...
- 将前端版本 `20260728084728-810eca3-eml-speed-eta` 部署到生产环境的 Mail Nest Docker Compose 服务，后端继续使用 `20260727165928-ba3ae39-eml-2g-stream`；更新前已备份远端 Mail Nest `docker-compose.yml`、`config.yaml` 和 `data/` 到 `backups/pre-20260728084728-eml-speed-eta.tgz`；线上健康检查、`/mail` 静态资源和前端容器镜像标签验证通过。
...
- 新增发送记录能力：后端新增 `mail_send_logs` 表和 `/api/v1/send-logs` 列表、详情接口，发信成功、SMTP 失败、已投递但本地保存失败都会记录状态、失败原因、收件人快照、SMTP Message-ID 和重试状态；前端新增“发送记录”页面，可按账号、状态、重试状态、关键词和日期筛选。
- 垃圾邮件规则增强：规则条件新增 `blacklist_sender`、`blacklist_domain`、`spam_keyword`，支持逗号、分号和换行分隔多个黑名单值；规则页新增“垃圾规则模板”，可一键创建移动到垃圾邮件的黑名单规则。
- 新增邮件会话视图：后端新增 `mail_threads` 表和 `mail_messages.thread_id`，同步新邮件时自动按 `source_message_id`、`References`、`In-Reply-To` 和同账号规范化主题归并会话；前端邮件页新增“邮件/会话”切换、会话列表、会话时间线和手动补齐会话入口。
- 新增规则命中记录：后端新增 `mail_rule_logs` 表和规则日志查询接口，规则执行时记录命中规则、动作、触发来源、条件快照、执行结果；规则列表返回命中次数、最近命中时间和最近结果；前端规则页和邮件详情新增规则记录抽屉。
- 新增草稿箱能力：后端新增 `mail_drafts` 表和草稿列表、详情、创建、更新、删除、发送接口；前端左侧系统文件夹新增草稿箱，写邮件、回复、回复全部和转发可自动保存草稿并继续编辑。
- 写信弹窗新增关闭、刷新和切换页面保护；发送失败时保留当前收件人、主题、正文、转发附件选择和本地已选附件，不会清空弹窗内容。
- 完成第一优先级能力设计文档，覆盖草稿箱、邮件线程聚合、规则命中记录，并补充图形验证码和系统管理设计。
- 登录和注册新增图形验证码：前端展示验证码图片并提交验证码 ID 和答案，后端使用短时内存验证码并在校验后立即失效。
- 新增系统管理用户页：管理员可查看用户总数、启用用户数、邮件量、附件占用以及每个用户的邮箱账号、邮件、附件、联系人、文件夹、规则、最近邮件和最近同步概览。
- 新增管理员用户停启用接口；首个注册用户自动成为管理员，旧库升级时若没有管理员则自动把最早用户设为管理员并保持启用，停用用户无法登录或继续使用旧 token 访问业务接口。

### 变更

- 规则动作“标记垃圾邮件”在前端文案中统一为“移动到垃圾邮件”，规则执行结果也返回“已移动到垃圾邮件”，更贴合邮件会从默认收件箱隐藏并进入垃圾邮件视图的行为。
- 邮件页完成第二阶段体验重构：邮件详情顶部改为摘要卡片，集中展示主题、时间、状态标签、发件人、收件人和抄送人，窄屏下回复/回复全部/转发自动收进更多菜单。
- 写邮件、回复、回复全部和转发弹窗重构为字段区、正文编辑区、附件区和固定底部操作区，正文区域独立承载富文本编辑，转发附件和本地附件展示更清晰。
- 邮件列表加载态改为邮件条目骨架屏，空状态根据“未配置邮箱、无启用账号、筛选无结果、当前文件夹无邮件”等场景显示不同操作建议。
- 邮件详情附件改为卡片网格展示，联系人弹层增加首字母头像、邮箱和补充信息层级，减少阅读区信息拥挤。
- 前端界面做第一轮视觉统一：内容页面板、页面标题区、登录/注册入口、邮件列表、写邮件弹窗和系统管理页统一为更克制的管理台风格。
- 邮件列表新增发件人首字母头像、未读指示点、状态标签和收件人摘要，提升列表扫读效率，并优化阅读区和写邮件弹窗的间距、固定底部操作区与附件卡片样式。
- 系统管理页新增前端搜索、启用状态筛选、停用用户数和邮箱账号总数统计，用户列增加头像标识，管理视图更接近可日常使用的控制台。
- 登录和注册页改为品牌视觉与表单并列的响应式布局，移动端自动收敛为单栏并保持验证码控件不溢出。
...
- MySQL 既有生产库升级时会先显式补齐 `users.is_admin` 和 `users.enabled`，再执行首个管理员修复，避免跳过 GORM AutoMigrate 的旧表因缺少管理员字段启动失败。
...
- MySQL 已有生产表时启动迁移不再执行可能缩列的 GORM `AutoMigrate`，只执行显式维护的类型保护和补充索引，避免升级时因历史长正文数据导致启动失败。
...
- 修复邮件详情页标题栏在操作按钮较多时被压缩过窄，导致长英文主题被拆成竖向多行、阅读区标题错乱的问题。
- 修复邮件列表头部在列表栏较窄时邮件数量被挤成竖排的问题，并优化邮件详情顶部回复、转发和规则记录工具栏的位置。

### 文档

- 更新 API 和架构文档，补充发送记录接口、`mail_send_logs` 数据模型、发信状态语义、垃圾邮件黑名单条件和 `mark_spam` 动作说明。
- 更新邮件线程与规则命中记录设计、API 规范和架构文档，记录会话归并策略、规则日志接口、`threadId` 字段、`mail_threads` 与 `mail_rule_logs` 数据模型。
- 更新草稿箱设计和 API 文档，记录草稿 CRUD、草稿发送、`draftId` 发信清理，以及本地附件跨刷新需要重新选择的浏览器限制。
- 新增邮件线程与规则命中记录详细设计，补充线程归并策略、规则日志模型、接口、前端交互、迁移策略和测试计划。
- 更新 API 和架构文档，补充验证码、管理员用户概览、用户状态字段和停用后的鉴权行为。
...
- 新增后端测试覆盖发信成功记录、SMTP 失败记录、黑名单发件人/域名/关键词规则自动移动到垃圾邮件和规则日志记录；后端 `rtk go test ./...` 通过，前端 `rtk npm run build` 通过。
- 新增后端 API 测试覆盖会话归并、跨邮箱账号同主题不误合并、会话重建、规则命中日志查询和规则列表命中统计；后端 `rtk go test ./...` 通过，前端 `rtk npm run build` 通过。
- 新增草稿存储和 API 测试，覆盖草稿保存、更新、列表、详情、删除和用户隔离；后端 `rtk go test ./...` 通过。
- 前端 `rtk npm run build` 通过，覆盖草稿箱、自动保存、离开保护和写信弹窗状态改造后的类型检查与生产构建。
- 前端 `rtk npm run build` 通过，覆盖邮件详情、写信弹窗、邮件列表骨架和空状态重构后的类型检查与生产构建。
- 前端 `rtk npm run build` 通过，并使用 Playwright 截图检查登录页桌面和移动端响应式布局。
- 新增后端测试覆盖验证码必填、验证码一次性使用、首个用户自动成为管理员、非管理员访问系统管理被拒绝、管理员停用用户、停用后旧 token 和登录均被拒绝。
- 扩展数据库迁移测试，覆盖 SQLite 旧库补齐管理员字段并将首个旧用户设为管理员，以及 MySQL 既有表显式补列定义。
- 后端 `rtk go test ./...` 通过，覆盖 GORM AutoMigrate 后的现有 storage、API 和 mail 服务行为。
- 新增 SQLite 旧库迁移回归测试，覆盖历史表存在但字段约束不完全一致时仍可安全启动。

### 部署

- 将前端版本 `20260723202423-5809fcd-mail-toolbar` 部署到生产环境的 Mail Nest Docker Compose 服务，后端继续使用 `20260723194532-b05b841-sendlogs-spam`；更新前已备份远端 `docker-compose.yml`、`config.yaml` 和 `data/` 到 `backups/pre-20260723202423-mail-toolbar.tgz`；线上健康检查、`/mail` 静态页面和前后端容器镜像标签验证通过。
- 将前端版本 `20260723200541-83c8be5-reader-title` 部署到生产环境的 Mail Nest Docker Compose 服务，后端继续使用 `20260723194532-b05b841-sendlogs-spam`；更新前已备份远端 `docker-compose.yml`、`config.yaml` 和 `data/` 到 `backups/pre-20260723200541-reader-title.tgz`；线上健康检查、`/mail` 静态页面和前后端容器镜像标签验证通过。
...
- 将后端版本 `20260723101611-aaec476-refactorlog` 部署到生产环境的 Mail Nest Docker Compose 服务；本地 Docker 构建 amd64 镜像后通过 `docker save | ssh docker load` 导入远端，更新前已备份远端 `docker-compose.yml`、`config.yaml` 和 `data/` 到 `backups/pre-20260723101611-aaec476-refactorlog.tgz`；线上健康检查、`/mail` 静态资源、后端容器镜像标签和启动日志验证通过。
