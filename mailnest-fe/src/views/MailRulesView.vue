      ...
      <a-table row-key="id" :columns="columns" :data-source="rules" :loading="loading" :pagination="false">
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'targetFolderId'">
            {{ folderName(record.targetFolderId) }}
          </template>
...
const columns: TableColumnsType<MailRule> = [
  ...
  { title: '目标文件夹', key: 'targetFolderId', width: 140 },
...
function openEdit(rule: MailRule) {
  ...
  form.targetFolderId = rule.targetFolderId;
...
async function saveRule() {
  if (!form.name.trim() || !form.targetFolderId || form.conditions.length === 0) {
    message.warning('请填写规则名称、目标文件夹和条件');
    return;
  }
  saving.value = true;
  try {
...
async function applyRules() {
  try {
    const result = await mailRuleApi.apply({ scope: 'unfiled' });
