...
批量导入 EML 时，业务限制为“单个 2GB、不限制文件个数”。浏览器前端采用“逐个文件 + 16MB 分片 + 上传会话心跳 + 断点续传”的方式导入：每个文件先创建可恢复上传会话，再按服务端返回的 `uploadedBytes` 续传；网络中断或浏览器请求失败后，用户重新选择同一批文件即可从服务端已接收的位置继续。旧版 multipart 导入接口仍保留兼容，但大量导入优先使用断点续传接口，避免十几 GB 文件集因为单个长请求中断而整体重来。
