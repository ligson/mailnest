...
批量导入 EML 时，业务限制为“单个 2GB、不限制文件个数”。前端会把大量文件自动拆成约 512MB 一批逐批上传，显示总进度、当前批次、已上传大小、已处理数量和成功/失败统计，单批上传失败会自动重试一次。前端容器 nginx 默认允许最大 `2304m` 请求体，用于覆盖最大单批文件量和 multipart 开销；如果外层还有 NAS 或网关反向代理，也需要配置不低于该值的上传上限，并建议对 `/api/` 开启 `proxy_request_buffering off` 和较长的读写超时，否则大批量导入可能在到达后端前返回 `413` 或转发中断。
