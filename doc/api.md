...
- `file`：`.eml` 文件，可重复传多个同名字段实现批量导入；不限制文件个数，单个文件最大 2GB，单次导入文件总大小最大 2GB。历史兼容字段名 `eml` 仍可使用。部署时前端 nginx 和外层反向代理的请求体上限应不低于 `2304m`，用于覆盖 2GB 文件总量和 multipart 开销，避免批量导入在到达后端前被拦截为 `413`。
