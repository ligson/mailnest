...
- `file`：`.eml` 文件，可重复传多个同名字段实现批量导入；接口层不限制文件个数，单个文件最大 2GB，单个请求内文件总大小最大 2GB。历史兼容字段名 `eml` 仍可使用。前端大量导入时会自动拆成约 512MB 一批逐批调用该接口，并展示上传进度、当前批次、已处理数量和导入统计。部署时前端 nginx 和外层反向代理的请求体上限应不低于 `2304m`，用于覆盖最大单批文件量和 multipart 开销；反向代理的 `/api/` 建议开启 `proxy_request_buffering off` 和较长读写超时，避免批量导入在到达后端前被拦截为 `413` 或转发中断。
